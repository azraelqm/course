...
/******/ 	__webpack_require__.p = "_includes/assets/";
/******/
/******/
...
__webpack_require__(/*! ../css/carousel.css */ "./src/css/carousel.css");
...
function Carousel(options) {
  ...
  */
  ...
  /*
  Matriz de eventos:
  contiene una matriz de objetos que contienen las referencias al elemento, nombre de evento y devolución de llamada para todos los oyentes de eventos agregados;
  esto se usa para eliminar posteriormente todos los enumeradores de eventos;
  */
  ...
  this.nextUntil = function (elem, selector, filter) {
    try {
      this._nextUntil(elem, selector, filter);
    } catch (error) {
      console.log(err);
    }
  };
...
/*
 * Establecer tamaño de fila:
 * ordena las opciones de tamaño de fila, si se usa la opción receptiva;
 * comprueba el tamaño de fila correcto en función de la ventana gráfica / opciones, y actualiza la opción de tamaño de fila en consecuencia
 */
...
/**
* *
* Establecer posición de fila:
* Para la fila de carrusel, establezca la posición x como atributo de referencia; también establece etiquetas de estilo translateX autoprefixed para posicionamiento
* @param {elRow}: elemento de fila
* @param {xPosition}: la posición x proporcionada utilizada para establecer la posición
* *
*/
...
/*
 * Establecer el ancho del artículo:
 * Agrega etiqueta de estilo para ancho%
 */
 ...
 * 
 * Ir:
 * solía ir al índice de elementos siguiente, anterior o específico;
 * @param {getWhere}: pase 'siguiente', 'anterior' o número de índice
 * 
 */
...
Carousel.prototype._goToItem = function (getWhere) {
  ...
  */
  ...
  /*
      - recorre todos los carruseles y artículos;
      - convierte la lista de nodos de elementos en matriz
      - si 'siguiente' o 'anterior'
      - filtra arrya según la posición x
      --- si es 'siguiente', filtre por elementos cuya posición x sea mayor que la posición actual
      --- si es 'anterior', filtre por elementos cuya posición x sea menor que;
      - si @parem es el número de índice, obtenga la posición x de ese índice (si el elemento existe) y vaya directamente
  */
  ...
  Array.prototype.forEach.call(self.selectors().elListCarousel, function (el, index) {
    ...
    /*
            ...
            si sigue, obtenga elementos cuya posición x sea mayor que la posición x de la fila
        */
    ...
    /*
            ...
            si es siguiente o anterior, obtenga la posición del primer elemento de la matriz filtrada y vaya a
        */
    ...
    if (getWhere === "next" || getWhere === "previous") {
      ...
      /*
          ...
          no need to use filtered-list, can use un-filtered list instead
            de lo contrario, si se pasa la posición del índice, simplemente vaya a la posición x de ese elemento específico;
          no es necesario usar una lista filtrada, puede usar una lista sin filtrar en su lugar
      */
...
Carousel.prototype._removeActiveItem = function () {
  Array.prototype.forEach.call(this.selectors().elListItems, function (el, index) {
    ...
    el.hasAttribute("data-carousel-active") && el.removeAttribute("data-carousel-active"); // el.getAttribute("data-carousel-move") && el.removeAttribute("data-carousel-move");
...
/**
    ...
    * La posición del elemento activo y sus hermanos depende de si el elemento está al principio, en el medio o al final;
    * en el elemento y la fila activos, se establece un atributo de posicionamiento y activo
    * *
*/
...
Carousel.prototype._initEventListeners = function () {
  var self = this;
  Array.prototype.forEach.call(this.selectors().elListItems, function (el, index, array) {
    self._addEventListener.call(this, el, "click", self._onclickEvent.bind(self)); //console.error(el);
    // mouseenter


    self._addEventListener.call(this, el, "mouseenter", self._mouseenterEvent.bind(self)); // mouseleave
...
/*
    ...
    crea objetos de eventos personalizados, empuja a la matriz de eventos;
    también aplica los oyentes de eventos
*/


Carousel.prototype._addEventListener = function (getEl, getEventName, getCallback) {
  var eventObj = {}; // build event object and push into event array for later removal if necessary
  //construir objeto de evento e insertarlo en la matriz de eventos para su posterior eliminación si es necesario

  eventObj.el = getEl;
  eventObj.eventName = getEventName;
  eventObj.callback = getCallback; // push into event array

  this.eventArray.push(eventObj); // add event to element
...
Carousel.prototype._onclickEvent = function (event) {
  var self = this;
  console.log("por fin ", event.target);
};
/**
 *
 * Mouseenter
...
Carousel.prototype._mouseenterEvent = function (event) {
  var self = this; // add mouseenter class

  event.target.classList.add("carousel-mouseenter"); // set timeout for slight delay before removing/adding active items; recheck if mouseenter class is still present (which is removed on mouseleave);

  window.setTimeout(function () {
    if (event.target.classList.contains("carousel-mouseenter")) {
      self._setActiveItem(event.target); // console.log("entro mouse");
...
Carousel.prototype._mouseleaveEvent = function (event) {
  var self = this; // remove mouseenter class, if present
...
/**
* Mostrar carrusel:
* necesario porque el usuario puede ocultarse opcionalmente antes de inicializar para evitar parpadeos de tamaño incorrecto, etc.
*/
 ...
 *
 */
//  NextUntil  Siguiente hasta


Carousel.prototype._nextUntil = function (elem, selector, filter) {
  console.log("clickeaste"); // Setup siblings array

  var siblings = []; // Get the next sibling element
  // Obtén el siguiente elemento hermano

  elem = elem.nextElementSibling;
  console.log("elem ", elem); // As long as a sibling exists
  // Mientras exista un hermano

  while (elem) {
    // If we've reached our match, bail
    // Si hemos llegado a nuestro partido, fianza
    if (elem.matches(selector)) break; // If filtering by a selector, check if the sibling matches
    // Si se filtra por un selector, verifique si el hermano coincide
...
__webpack_require__(/*! ../css/browse.css */ "./src/css/browse.css");

__webpack_require__(/*! ../css/single.css */ "./src/css/single.css");

var _carousel = __webpack_require__(/*! ./carousel.js */ "./src/js/carousel.js");

var Carousel = _carousel.Carousel;
...
let videoheader = document.getElementById("hero-video"); // if (videoheader !== undefined){
...
let btnPrev = document.querySelector(".btnPrev"); //let imgn = document.querySelector(".car_item img");
...
carouselas.initialize(); // carouselas.goTo("7") // opcion para ir a un item exacto

btnNext.addEventListener("click", e => {
  console.log("clickeaste"); //carousel.goto("next");
...
btnPrev.addEventListener("click", e => {
  ...
  //carousel.goto("next");
  carouselas.goTo("previous"); //   carouselas.reInitialize({ //opcion ppara reiniciar
  //     selector: ".car",
  //     rowSize: 5,
  //     responsive: [
  //         {
...
carousel2.initialize(); // ---------- Fin de carousel
...
__webpack_require__(/*! D:\netrep\course\src\js\index.js */"./src/js/index.js");
module.exports = __webpack_require__(/*! D:\netrep\course\src\css\index.css */"./src/css/index.css");
